...
// Reproduce the exact binning logic from original kmer_count.cpp
size_t original_style_binning(const std::string& kmer_str, size_t num_bins) {
    // Convert k-mer string to binary representation (2 bits per nucleotide)
    std::string bit_string = "";
    for (char c : kmer_str) {
        switch (c) {
            case 'A': case 'a': bit_string += "00"; break;
            case 'C': case 'c': bit_string += "01"; break;
            case 'G': case 'g': bit_string += "10"; break;
            case 'T': case 't': bit_string += "11"; break;
            default: bit_string += "00"; break; // Handle N or other
        }
    }
    
    // Take first 64 bits (32 nucleotides) like original code
    if (bit_string.length() > 64) {
        bit_string = bit_string.substr(0, 64);
    }
    
    // Convert to unsigned long like original: bitset<2*k>(key.to_string().substr(0,64)).to_ulong()
    uint64_t key_hash = 0;
    for (size_t i = 0; i < bit_string.length(); i++) {
        key_hash = (key_hash << 1) + (bit_string[i] - '0');
    }
    
    return key_hash % num_bins;
}
...
void process_kmc_database(const string& kmc_db_path, const string& output_path, 
    ...
    while (kmc_db.ReadNextKmer(kmer, count)) {
        ...
        // Using the exact same binning logic as original kmer_count.cpp
        size_t bin_idx = original_style_binning(kmer_str, num_bins);
